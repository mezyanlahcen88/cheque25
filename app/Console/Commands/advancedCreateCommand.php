...
{
    protected $signature = 'make:create {table} {model}';
    ...
    public function handle()
    {
        ...
        $formContent = $this->generateFormContent($table, $columns, $lowerName, $object);
    ...
    private function generateFormContent($table, $columns, $lowerName, $object)
    {
            ...
            if (strpos($column, '_id') !== false) {
                ...
                $formContent .= <<<EOT
                                            ...
                                            <x-single-select cols="col-md-6" div-id="{$column}" column="{$column}" model="{$lowerName}"
                                                label="{$lowerName}_form_{$column}" optional="{$optionalClass}" id="{$column}" :options="{$option}()" :object=false />
                EOT;
            } elseif (in_array($column, $avatarColumns)) {
                $hasAvatarColumn = true;
                $avatarContent .= <<<EOT

                                        <x-image-field :background-url="url('/assets/media/svg/avatars/blank.svg')" :image-url="url('/assets/media/avatars/300-1.jpg')" avatar-name="{$column}" model="{$lowerName}"/>
                EOT;
                ...
                $formContent .= <<<EOT
                                            ...
                                            <x-input-field cols="col-md-6" divId="{$column}" column="{$column}" model="{$lowerName}"
                                                ...
                                                columnValue="{{ old('{$column}') }}" attribute="unique:{$table}" readonly="false" />
                EOT;
                ...
                $formContent .= <<<EOT
                                            ...
                                            <x-input-field cols="col-md-6" divId="{$column}" column="{$column}" model="{$lowerName}"
                                                optional="{$optionalClass}" inputType="{$inputType}" className="" columnId="{$column}"
                                                columnValue="{{ old('{$column}') }}" attribute="{$requiredAttribute}" readonly="false" />
                EOT;
            }
        }
                        ...
                        <div class="col-md-12">
                            <div class="card card-bordered">
                                <div class="card-header">
                                    <h3 class="card-title">{{trans('translation.{$lowerName}_action_add')}}</h3>
                                </div>
                                <div class="card-body">
                                    <div class="row">
