    <div class="d-flex justify-content-center gap-2">
        @can('secteur-restore')
        <form action="{{ route('secteur.restore', $object->id)}}" method="POST" id="restoreForm">
            @csrf
            @method('PUT')
        ...
        @endcan
        @can('secteur-force-delete')
            <div class="remove">
                <a href="#" class="remove-item-btn" data-bs-toggle="modal"
                data-id="{{ $object->id }}" data-route-name="{{ route('secteur.forceDelete', $object->id) }}">
