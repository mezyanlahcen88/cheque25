...
namespace Modules\Cheque\App\Http\DataTable;

use Modules\Cheque\App\Models\Cheque;

class ChequeDataTable {
...
->filterColumn('carnet_id', function($query, $keyword) {
    ...
    if ($keyword === '0' || $keyword === '1') {
        $query->where('isactive', $keyword);
            ...
            ->addColumn('actions', function (Cheque $object) {
                return view('cheque::actions', compact('object'));
            })
            ->addColumn('checkbox', function (Cheque $object) {
                return view('components.checkbox', compact('object'));
            })
        ->addColumn('isactive', function ($object) {
    return view('components.isactive', ['object' => $object, 'lowerName' => 'cheque']);
})
...
->filterColumn('carnet_id', function($query, $keyword) {
    ...
    if ($keyword === '0' || $keyword === '1') {
        $query->where('isactive', $keyword);
            ...
            ->addColumn('actions', function (Cheque $object) {
                return view('cheque::trashedActions', compact('object'));
            })
            ->addColumn('checkbox', function (Cheque $object) {
                return view('components.checkbox', compact('object'));
            })
