<div class="{{ $cols }} my-1" id="{{ $divID ?? '' }}">
    <div class="form-group">
        ...
        <select class="form-select @error($column) is-invalid @else is-valid @enderror" data-control="select2" multiple="multiple" data-allow-clear="true"
            ...
            {{-- @foreach ($options as $key => $value)
                ...
                @endif
            @endforeach --}}
            @foreach(project_types() as $parent => $childs)
            {{-- @if ($object) --}}
