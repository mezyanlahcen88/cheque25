...
$('#state_id').on('change', function() {
    datatable.column(8).search($(this).val()).draw();
});
$('#city_id').on('change', function() {
    datatable.column(9).search($(this).val()).draw();
});
$('#secteur_id').on('change', function() {
    datatable.column(10).search($(this).val()).draw();
});
$('#isactive').on('change', function() {
    datatable.column(16).search($(this).val()).draw();
});
            }
        });
    }
     ...
     var exportButtons = function() {
        const documentTitle = 'suppliers Report';
